    ...
    public function getTodayAttentions()
    {
        ...
        $attentions = $query->get()->map(function ($attention) {
            if ($attention->person_type == '001') {
                ...
                $attention->person_name = Professor::select(DB::raw("CONCAT_WS(' ', name,paternal_surname) as name"))->where('id', $attention->person_id)->first()->name;
                ...
                $attention->unit_name = Office::select('name')->where('id', Worker::select('office_id')->where('id', $attention->person_id)->first()->office_id)->first()->name ?? 'Sin oficina asignada';
